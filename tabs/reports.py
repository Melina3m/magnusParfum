...
def render_reports(db):
    ...
    def get_sale_data(sale):
        # Intentar obtener costo guardado en la venta, si no, ir al inventario
        ...
        # Determinar si es INV (Prioridad al flag de la venta, luego al del producto)
        is_inv = sale.get("inv")
        if is_inv is None:
    ...
    with st.expander("Ver detalle de ventas en el periodo"):
        if fsales:
            ...
            # Limpiar columnas para el usuario
            cols = ["date", "customer", "quantity", "unit_price", "payment", "inv"]
            st.dataframe(df_display[cols] if all(c in df_display for c in cols) else df_display, use_container_width=True)
        else:
